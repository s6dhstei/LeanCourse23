...
lemma pflaumenmus (s : StrictBipointedType) {z : s.S} : z ≠ s.x₁ ∨ z ≠ s.x₀ := by {
  by_cases h : z = s.x₁
  · right
    subst z
    apply Ne.symm
...
class StrictBipType (S : Type*) where
  x0 : S
  x1 : S
  neq : x0 ≠ x1

instance : StrictBipType ℤ where
  x0 := 0
  x1 := 1
  neq := by exact Int.zero_ne_one




#check StrictBipType.x0
-- notation " y " => StrictBipType.x1
lemma quetschenkompott (S : Type*) [StrictBipType S] (z : S) : z ≠ StrictBipType.x0 ∨ z ≠ StrictBipType.x1 := by {
  by_cases h : z = StrictBipType.x1
  · left
    subst z
    exact Ne.symm StrictBipType.neq
  · right
    exact h
}
