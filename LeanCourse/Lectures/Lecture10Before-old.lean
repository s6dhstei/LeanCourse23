...
theorem units_ne_neg_self [Ring R] [CharZero R] (u : Rˣ) : u ≠ -u := by {
  simp
  intro h
  have g : u * u⁻¹ = -u * u⁻¹ := by exact congrFun (congrArg HMul.hMul h) u⁻¹
  simp at g
  have f : ((1 : Rˣ) : R) + 1 = -1 + 1 := by rw[g]; push_cast; rfl
  simp at f
  norm_num at f
}
...
example (n m : ℤ) : span {n} ⊔ span {m} = span {gcd n m} := by {
  ...
  constructor
  intro hx
  refine mem_span_singleton.mpr ?h.mp.a
  have hn : n∣x := by sorry
  sorry
