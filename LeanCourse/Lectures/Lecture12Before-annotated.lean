...
example {f : ℝ → ℝ} (hf : Continuous f) (h2f : Injective f) {a b x : ℝ}
    (hab : a ≤ b) (h2ab : f a < f b) (hx : x ∈ Icc a b) : f a ≤ f x := by {
      by_contra g
--      have cab : ContinuousOn f (Icc a b) := by exact Continuous.continuousOn hf
      have ivt : _ := intermediate_value_Icc hab (Continuous.continuousOn hf)
      simp at g
      have fxoutside : ¬ f x ∈ Icc (f a) (f b) := by exact not_mem_Icc_of_lt g
      have fxinside : f x ∈ f '' Icc a b := by exact mem_image_of_mem f hx
      sorry
    }
